...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.vu7vy.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    const achievementsCollection = client.db('farees').collection('achievement');
    const clientCollection = client.db('farees').collection('client');
    const certificatesCollection = client.db('farees').collection('certificates');
    const missionVisionCollection = client.db('farees').collection('missionVision');
    const areaActivitesCollection = client.db('farees').collection('areaActivites');
    ...
    app.patch('/missionVisionUpdate/:id', async (req, res) => {
      ...
      try {
        const result = await missionVisionCollection.updateOne(query, updateDoc);
        res.send(result);
      } catch (error) {
        console.error("Error updating data:", error);
        res.status(500).send({ error: "Failed to update data" });
    ...
    app.get("/jobs/:id", async (req, res) => {
      ...
      try {
        const result = await careerCollection.findOne({ _id: new ObjectId(id) });
        if (!result) {
